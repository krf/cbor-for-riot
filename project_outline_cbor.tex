...
\title{Project Outline: Implementation of CBOR for RIOT}
%
%
...
\IEEEPARstart{A}{CBOR} (Concise Binary Object Representation) implementation for RIOT will make it possible for devices running RIOT to exchange serialized, compressed binary objects, which in turn could serve several purposes, such as remote procedure calls, message passing, or generally exchanging C data items to be processed on another RIOT device.\\
...
\subsubsection{Get our initial code to run on RIOT}
\subsubsection{Define API for our CBOR library}
\subsubsection{Implement and test the CBOR encoder and decoder for simple types}
\subsubsection{Implement and test the CBOR encoder and decoder for complex types}
\subsubsection{Test our implementation against other working implementations}


\subsection{Subtasks}
\subsubsection{Writing a simple module + header, placing them in the appropriate directories, calling them from within a RIOT process}
\subsubsection{Define API for our CBOR library}
\subsubsection{Implement serialization and deserialization functions for simple types (PODs, such as int, float, bool, ...)}
Test the (de)serialization code against each other. Check if serializing + deserializing an object yields the same object.
\subsubsection{Implement serialization and deserialization functions for complex types (such as arrays, maps)}
...
1. Reserving a roughly estimated amount of buffer upfront which can hold the serialized data\\
...
3. Incrementally re-allocate the buffer holding the serialized data while doing the serialization\\


\subsection{Test our implementation against other working implementations}
...
\section{Approach}
\subsection{Serializing/Deserializing POD types}
Suggested API for **cbor.h** (for simple types):\\
\begin{lstlisting}
// POD: int
// Return bytes written into @p dest
size_t cbor_serialize(
char* dest, int* input)
{
// Write dest
}
/// Return bytes written into @p dest
size_t cbor_deserialize(
const char* src, int* dest)
{
// Write dest
}
// POD: float (...)
...
size_t finalSize = 0;
char dest[16]; 
// reserve big enough buffer (upper bound)
...
\subsection{Milestone 1: May 13th}
...
\subsection{Milestone 2: June 17th}
...
\subsection{Milestone 3: July 1st}
...
- Implement and test the CBOR encoder and decoder for:\\
	* unsigned integers, negative integers: Kevin\\
	* byte strings, unicode strings: Jana\\
